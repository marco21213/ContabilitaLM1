...
# ADERISCO AL SERVIZIO - NA CAMURRIA!
    ...
    #========================================================================================
        ...
        for f in os.listdir('.'):
            if f.startswith('fatturaID') and f.endswith('.json'):
                os.remove(f)
    ...
    # Pulizia dei file JSON anche in caso di interruzione
    for f in os.listdir('.'):
        if (f.startswith('fe_') or f.startswith('fatturaID')) and f.endswith('.json'):
            os.remove(f)

# In ScarFec31.py, alla fine del file prima di sys.exit():

try:
    # Costruisci il percorso corretto per lo script
    script_path = os.path.join('scripts', 'converti_xml.py')
    
    if VenOAcq == "A":  # Solo acquisti
        subprocess.run(['python', script_path, '--tipo', 'acquisti'])
    elif VenOAcq == "V":  # Solo vendite
        subprocess.run(['python', script_path, '--tipo', 'vendite'])
    else:  # Entrambi
        subprocess.run(['python', script_path, '--tipo', 'tutti'])
except Exception as e:
    print(f"Errore durante la conversione P7M: {e}")
sys.exit()
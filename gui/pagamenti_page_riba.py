...
class RibaTab(tk.Frame):
    ...
    def create_table(self):
        ...
        columns = ("SELEZIONA", "CLIENTE", "DOCUMENTO", "DATA SCADENZA", "IMPORTO", "STATO", "DISTINTA", "DATA DISTINTA", "PDF")
        ...
        for col in columns:
            if col != 'SELEZIONA':  # La colonna checkbox non ha filtro
        ...
        widths = [("CLIENTE", 200), ("DOCUMENTO", 120), ("DATA SCADENZA", 110), ("IMPORTO", 90),
                  ("STATO", 100), ("DISTINTA", 100), ("DATA DISTINTA", 100), ("PDF", 50)]
    ...
    def apply_filters(self):
        ...
        for row in self.original_data:
            ...
            for k, v in self.active_filters.items():
                ...
                if k == 'importo':
                    try:
                        if float(v) != float(row[k]): ok = False; break
                    except:
                        ok = False; break
                elif k == 'data_scadenza':
                    if '|' in v:
                        parti = v.split('|')
                        dal = parti[0].strip() if len(parti) > 0 else ''
                        al = parti[1].strip() if len(parti) > 1 else ''
                        
                        data_row = str(row.get(k, ''))
                        if not data_row:
                            if dal or al:
                                ok = False
                                break
                            ...
                            try:
                                if '/' in data_row:
                                    parts_row = data_row.split('/')
                                    if len(parts_row) == 3:
                                        data_row_comp = (int(parts_row[2]), int(parts_row[1]), int(parts_row[0]))
                                    else:
                                        data_row_comp = None
                                else:
                                    data_row_comp = None
                                ...
                                if dal:
                                    parts_dal = dal.split('/')
                                    if len(parts_dal) == 3:
                                        data_dal_comp = (int(parts_dal[2]), int(parts_dal[1]), int(parts_dal[0]))
                                        if data_row_comp and data_row_comp < data_dal_comp:
                                            ok = False
                                            break
                                    else:
                                        if dal.lower() not in data_row.lower():
                                            ok = False
                                            break
                                ...
                                if al:
                                    parts_al = al.split('/')
                                    if len(parts_al) == 3:
                                        data_al_comp = (int(parts_al[2]), int(parts_al[1]), int(parts_al[0]))
                                        if data_row_comp and data_row_comp > data_al_comp:
                                            ok = False
                                            break
                                    else:
                                        if al.lower() not in data_row.lower():
                                            ok = False
                                            break
                            except:
                                if dal and dal.lower() not in data_row.lower():
                                    ok = False
                                    break
                                if al and al.lower() not in data_row.lower():
                                    ok = False
                                    break
                    else:
                        if v not in str(row.get(k, '')).lower():
                            ok = False
                            break
                elif v not in str(row.get(k, '')).lower():
                    ok = False; break
            if ok: res.append(row)
        ...
        for n, r in enumerate(data):
            ...
            item_id = self.tree.insert("", tk.END, 
                values=[checkbox_val, r['cliente'], r['documento'], r['data_scadenza'],
                        f"€ {r['importo']:,.2f}", r['stato'], r['distinta'], r['data_distinta'], pdf_flag],
    ...
    def show_filter_menu(self, field_name, column_title):
        """Mostra il menu per i filtri RiBa"""
        ...
        if field_name == 'data_scadenza':
            ...
            tk.Label(frame, text="Dal (dd/mm/yyyy):", bg=Style.WHITE,
                     fg="#000", font=("Arial", 9)).pack(anchor='w', pady=(0, 5))
            entry_dal = tk.Entry(frame, font=("Arial", 10), width=20)
            filtro_attuale = self.active_filters.get(field_name, "")
            if "|" in filtro_attuale:
                parti = filtro_attuale.split("|")
                entry_dal.insert(0, parti[0] if len(parti) > 0 else "")
            entry_dal.pack(pady=(0, 10))

            tk.Label(frame, text="Al (dd/mm/yyyy):", bg=Style.WHITE,
                     fg="#000", font=("Arial", 9)).pack(anchor='w', pady=(0, 5))
            entry_al = tk.Entry(frame, font=("Arial", 10), width=20)
            if "|" in filtro_attuale:
                parti = filtro_attuale.split("|")
                entry_al.insert(0, parti[1] if len(parti) > 1 else "")
            entry_al.pack(pady=(0, 15))

            def apply_date_range():
                dal = entry_dal.get().strip()
                al = entry_al.get().strip()
                if dal or al:
                    filtro_val = f"{dal}|{al}"
                else:
                    filtro_val = ""
                self.active_filters[field_name] = filtro_val.lower()
                self.apply_filters()
                win.destroy()

            tk.Button(frame, text="Applica", bg="#4CAF50", fg="white",
                      width=10, command=apply_date_range).pack(side="left", padx=5)
            tk.Button(frame, text="Chiudi", bg="#f44336", fg="white",
                      width=10, command=win.destroy).pack(side="left")
        else:
            ...
            tk.Button(frame, text="Applica", bg="#4CAF50", fg="white",
                      width=10, command=apply_text).pack(side="left", padx=5)
            tk.Button(frame, text="Chiudi", bg="#f44336", fg="white",
                      width=10, command=win.destroy).pack(side="left")
    ...
    def on_tree_click(self, event):
        ...
        if col == '#9':
            ...
            if distinta_id:
                ...
                messagebox.showinfo('Info', 'Questa Ri.Ba. non è associata a nessuna distinta')
